...
class Logger:
    ...
    def success(text: str) -> None:
        ...
        try:
            Logger.__logs[f"{Logger.__date} {Logger.__current_time}"].append(f"SUCCESS | {text}")
        except KeyError:
            Logger.__logs[f"{Logger.__date} {Logger.__current_time}"] = [f"SUCCESS | {text}"]
    ...
    def info(text: str) -> None:
        ...
        try:
            Logger.__logs[f"{Logger.__date} {Logger.__current_time}"].append(f"INFO | {text}")
        except KeyError:
            Logger.__logs[f"{Logger.__date} {Logger.__current_time}"] = [f"INFO | {text}"]
    ...
    def error(text: str) -> None:
        ...
        try:
            Logger.__logs[f"{Logger.__date} {Logger.__current_time}"].append(f"ERROR | {text}")
        except KeyError:
